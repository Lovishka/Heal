...
const HospitalDashboard = () => {
  ...
  const [patientQueue, setPatientQueue] = useState([
    ...
    { id: 'P103', name: 'Priya Sharma', status: 'Admitted', unit: 'General Ward' },
    { id: 'P104', name: 'Aman Khan', status: 'Admitted', unit: 'ICU' },
    { id: 'P105', name: 'Kavita Iyer', status: 'Admitted', unit: 'Pediatrics' },
  ]);
  ...
  const [updateFormData, setUpdateFormData] = useState(userProfile);
  ...
  const approveBooking = async (patientId) => {
    try {
      ...
      setPatientQueue(prevQueue =>
        prevQueue.map(p =>
          p.id === patientId ? { ...p, status: 'Admitted' } : p
        )
      );
  ...
  const dischargePatient = async (patientId) => {
    const patientToDischarge = patientQueue.find(p => p.id === patientId);
    ...
    try {
      ...
      setBedStatus(prevStatus => {
        const unit = patientToDischarge.unit.toLowerCase().replace(/\s/g, '');
        if (prevStatus[unit]) {
          const updatedUnit = {
            ...prevStatus[unit],
            occupied: prevStatus[unit].occupied - 1
      ...
      alert(`${patientToDischarge.name} has been successfully discharged.`);
    } catch (error) {
  ...
  const handleAddPatientSubmit = (e) => {
    ...
    setPatientQueue(prevQueue => [newPatient, ...prevQueue]);
    setDashboardData(prevData => ({
      ...prevData,
      pendingBookings: prevData.pendingBookings + 1
    }));
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-xl p-10">
        ...
        <section className="bg-white p-8 rounded-lg shadow-lg border border-gray-200">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-gray-800">Bookings & Patient List</h3>
          ...
          {filteredPatients.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPatients.map(patient => (
                <div key={patient.id} className="bg-gray-50 p-6 rounded-lg shadow-sm border border-gray-200">
                  ...
                  <p className="text-sm text-gray-500 mb-4 font-bold">{patient.status}</p>
                  <div className="flex space-x-2">
                    ...
                    {patient.status === 'Admitted' && (
                      <button
                        ...
                        className="w-full bg-red-600 text-white font-bold py-2.5 rounded-lg shadow hover:bg-red-700 transition duration-300"
                      >
                        Discharge Patient
                      </button>
                    )}
            ...
            <p className="text-center text-gray-500 py-6 text-lg">
              🎉 All clear! No patient records match the search criteria.
            </p>
          )}
        </section>
      </div>
